...
class AgentToolFactory:
    ...
    def create_search_tool(self):
        ...
        async def search(state: "RunContextWrapper[SessionState]", question: str) -> str:
            ...
            print(f"Starting paper search for question: {question}")
            ...
            try:
                ...
                if not stats or stats.get("total_documents", 0) == 0:
                    print("No vector index found. Creating index...")
                    self.rag_engine.index_articles(collection=self.config.collection_filter)
                    print("Index created successfully!")
            except Exception as e:
                print(f"Creating vector index: {e}")
                self.rag_engine.index_articles(collection=self.config.collection_filter)
            ...
            print(f"Paper search returned {count_results} passages from papers")
            self._print_status(state.context.status)
            
            return f"Found {count_results} text passages from the papers that semantically matches and can help answer the question."
    ...
    def create_evidence_tool(self):
        ...
        async def gather_evidence(state: "RunContextWrapper[SessionState]", question: str) -> str:
            ...
            print(f"Gathering evidence for question: {question}")
            ...
            print(f"Finished gathering evidence for question: {question}")
            ...
            return f"Found and added {count_top_evidence} pieces of evidence relevant to the question. Best evidences: {best_evidence}."
